...
class OpenNFCSense4P implements PConstants {
   ...
   */
  public OpenNFCSense4P(String fileName) {
    initNFCBits(fileName);
    welcome();
  }

  public OpenNFCSense4P(String fileName, float sample_rate) {
    initNFCBits(fileName);
    SAMPLE_RATE = sample_rate;
    welcome();
  }

  public OpenNFCSense4P(String fileName, float sample_rate, int timer1, int timer2) {
    initNFCBits(fileName);
    SAMPLE_RATE = sample_rate;
  ...
  private void welcome() {
    System.out.println("Open NFCSense for Processing 0.0.2 by Rong-Hao Liang r.liang@tue.nl");
  }
  ...
  void initNFCBits(String fileName) {
    Table csvData = loadTable(fileName, "header");
    nbits = new ArrayList();
    if (csvData != null) {
  ...
  void initParam() {
    fontList = PFont.list();
    font = createFont("Lucida Sans Regular", 72);
    textFont(font);
    gH = height;
    gW = width;
    last_t1 = millis();
  ...
  public void drawResults() {
    ...
    for (int i = 0; i < colNum; i++)
      line(0, i * height / colNum, width, i * height / colNum);
    popStyle();
  }
  ...
  void barGraph(float[] data, float _x, float _y, float _w, float _h) {
    int colors[] = { color(155, 89, 182), color(63, 195, 128), color(214, 69, 65), color(82, 179, 217), 
      color(52, 73, 94), color(242, 121, 53), color(0, 121, 53), color(128, 128, 0), color(52, 0, 128), 
      color(128, 52, 0) };
    pushStyle();
    noStroke();
  ...
  void lineGraph(float[] data, float _l, float _u, float _x, float _y, float _w, float _h, int _index) {
    int colors[] = { color(100, 0, 0), color(0, 100, 0), color(0, 0, 100), color(100, 0, 100), color(100), };
    int index = PApplet.min(PApplet.max(_index, 0), colors.length);
    pushStyle();
    float delta = _w / (data.length - 1);
    beginShape();
  ...
  //2.0
  public void printLastBitID() { //print the latest tag appearance
    if (lastIDList.size()>0) {
      println(lastIDList.get(lastIDList.size()-1));
    }
  }
  public int getLastBitID() { //print the latest tag appearance
    int lastID = -1;
    if (lastIDList.size()>0) {
  ...
  public void printRecentBitIDs() { //print the latest tag appearance
    for (int i=lastIDList.size()-1; i>=0; i--) {
      print(lastIDList.get(i));
      print(',');
    }
    println();
  }
  ...
  public int searchID(ArrayList<Long> id_db, long id) {
    int index = -1;
  ...
  public void drawMotionModeRecords(float x, float y, float gW, float gH) {
    ...
    rect(x, y, gW, gH);
    textAlign(RIGHT, TOP);
    textSize((float)(0.6* gH / (float)(MotionTypeNum)));
    for (float i=1; i<10; i++) {
      float dx = (float)(i*(float)gW/10.);
      stroke(100);
      noFill();
      line(x+dx, y, x+dx, y+gH);
      noStroke();
      fill(100);
      text(nf((float)(1.0-i/10.), 0, 1)+"s", x+dx-5, y+5);
    }
    textAlign(RIGHT, BOTTOM);
    text("motion mode history (last second)", x+gW-5, y-5);

    noStroke();
    textAlign(RIGHT, TOP);
    for (int n=0; n<MotionTypeNum; n++) {
      fill(colors[n]);
      text(mTypeItems[n], x+gW-5, y + n * gH / (float)(MotionTypeNum)); // history
    }
    popStyle();
  }
  ...
  public void checkTagID(int index) { // called in Serial so that it won't be limited by the 60fps refresh rate
  ...
  void updateOnTimers(int id_read) { // t1, t2, and idOnCounters
    ...
    if (id_read >= 0) { // id_read>=0
      ...
      ttl2 = millis() - last_t2; // start counting t2
